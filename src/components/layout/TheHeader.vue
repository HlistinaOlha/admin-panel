<template>
  <header>
    <div class="container d-flex flex-column align-items-start justify-content-between">
      <div class="mt-3 d-flex justify-content-between w-100 align-items-center">
        <h1 class="h1">
          <router-link class="nav-link" to="/">
            <img alt="robot"
                 class="robot"
                 src="data:image/png;base64,">
            Admin Panel
          </router-link>
        </h1>
        <div v-show="!isProduction" class="test-mode">This is the test mode =)</div>
      </div>
    </div>
  </header>
</template>

<script>

export default {
  name: 'TheHeader',
  computed: {
    isProduction() {
      return JSON.parse(process.env.VUE_APP_IS_PRODUCTION)
    }
  }
}
</script>

<style lang="scss" scoped>
header {
  width: 100%;
  height: 70px;
  font-size: .875rem;
  background: $link-hover-color;
  display: flex;
  align-items: center;
  justify-content: center;
  border-bottom: 1px solid $main-border-color;
}

header .nav-link {
  cursor: pointer;
  padding: 0;
  display: flex;
  align-items: center;
  color: #6d7080;
}

.nav-link span {
  font-weight: $font-weight-normal;
}

h1 {
  margin: 0;
}

h1 .nav-link {
  font-size: 21px
}

.nav-link {
  margin: 0;
  padding: 15px 10px;
  font-weight: $font-weight-medium;
}

h1 .nav-link:hover,
h1 .nav-link:active,
h1 .nav-link.router-link-active {
  border-color: transparent;
}

.robot {
  vertical-align: bottom;
  margin-right: 10px;
}

.test-mode{
  color: $footer-text-color;
}

@keyframes dropdownAnimation {
  0% {
    opacity: 0;
    -webkit-transform: translate3d(0, -30px, 0);
    transform: translate3d(0, -30px, 0);
  }
  100% {
    opacity: 1;
    transform: none;
    transform: translateZ(0);
  }
}
</style>
